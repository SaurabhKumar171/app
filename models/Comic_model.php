...
class Comic_model extends CI_Model {
    ...
    public function avatar_icons($num) {
        $this->db->trans_start();
        $query = $this->db->query("select * from tbl_avatars_new where type='0' or type='$num' order by img_order ASC");
        $row = $query->result();
        return $row;
    }
    ...
    public function getMyAvatar($mobnum) {
        $this->db->trans_start();
        $query = $this->db->query("select avatarURL from `tbl_saved_avatars` WHERE mobnum='$mobnum' order by id desc limit 5");
        $row = $query->result();
        return $row;

        // $query = $this->db->query("SELECT avatarURL FROM `tbl_saved_avatars` WHERE mobnum='$mobnum' order by id desc");
        // $array = $query->row_array();
        // return $array;
    }
...
(`mobnum`,`narration`,`backImage`, `leftComment`, `rightComment`, `leftImg`, `rightImg`, `saved_avatars_id`, `date`,`leftBubble`,`rightBubble`) 
    ...
    public function storyImages($charId) {
        $this->db->trans_start();
        $query = $this->db->query("select * from `tbl_story_character` where create_story_id = '$charId' order by id desc");
        $row = $query->result();
        return $row;
    }
...
(`mobnum`,`narration`,`backImage`, `leftComment`, `rightComment`, `leftImg`, `rightImg`, `saved_avatars_id`, `date`,`leftBubble`,`rightBubble`) 
    ...
    public function updateCartoon($fimg,$simg,$gen_id) {
        if($fimg==''){
            $sql = "update `tbl_cartoon_res` set status='1',`final_img`='".$fimg."',`final_img_thumb` ='".$simg."' where `id` = '$gen_id'";
            $this->db->query($sql);
        }else{
    ...
    public function phoCartoonEmoji($status) {
        $sql = "select * from tbl_cartoonist_bck3march2018 where status='$status'";
        $query = $this->db->query($sql);
        $row = $query->result();
        return $row;
    }
    ...
    public function finalResultCartooniest($mobnum){
        //$sql = "select * from tbl_cartoon_res where mobnum='$mobnum' and id='$id'";
        $sql = "select * from tbl_cartoon_res where mobnum='$mobnum' and final_img!='' order by id desc limit 1";
        $query = $this->db->query($sql);
        $array = $query->row_array();
        return $array;
    }
    ...
    public function confrimTosaveEmojiCartoon($mobnum) {
        $sql = "update `tbl_cartoon_res` set `status`='0' where mobnum = '$mobnum' and final_img!='' and effect_type='1' order by id desc limit 1";
        $this->db->query($sql);
    }
